...
class ScatterLegContext implements ProxyContext {
    ...
    private Future<Void> captureResponse() {
        if (response.getBody() == null) {
            set("scatter.leg.body", Buffer.buffer());
            set("scatter.leg.statusCode", response.getStatusCode());
            set("scatter.leg.headers", MultiMap.caseInsensitiveMultiMap().addAll(response.headers()));
            return Future.succeededFuture();
        }
        return BodyAccumulator.readBodyBuffer(response.getBody()).map(buffer -> {
            set("scatter.leg.body", buffer);
            set("scatter.leg.statusCode", response.getStatusCode());
            set("scatter.leg.headers", MultiMap.caseInsensitiveMultiMap().addAll(response.headers()));
            return null;
        });
    ...
    Future<LegResponse> execute() {
        return sendRequest()
                ...
                .map(v -> new LegResponse(
                        originIdentifier,
                        get("scatter.leg.body", Buffer.class),
                        get("scatter.leg.statusCode", Integer.class) != null
                                ? get("scatter.leg.statusCode", Integer.class)
                                : -1,
                        get("scatter.leg.headers", MultiMap.class) != null
                                ? get("scatter.leg.headers", MultiMap.class)
